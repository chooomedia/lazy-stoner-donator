class AdCard extends Card {
  constructor(cardOptions = {}) {
    ...
    this.badgeLabel = cardOptions.badgeLabel;
    this.badgeTitle = cardOptions.badgeTitle;
    ...
    this.statusLabel =
      cardOptions.statusLabel || cardContent.giftedLabel || "Schon geschenkt";
    this.statusMeta =
      cardOptions.statusMeta ||
      cardContent.giftedMeta ||
      ...
      if (event.key === "Escape") {
        const activeShare =
          document.activeElement && document.activeElement.closest
            ...
            : null;
        const activeButton = activeShare
          ? activeShare.querySelector("button[aria-expanded]")
          : null;
        AdCard.closeOpenShareMenus();
        activeButton?.focus();
        return;
  ...
  static getShareMenuItems(wrapper) {
    ...
    return Array.from(
      wrapper.querySelectorAll('.product-share-menu [role="menuitem"]'),
    );
  }
  ...
  createProductUrl(adId) {
    return (
      "https://www.amazon.de/dp/" + adId + "/ref=nosim?tag=" + this.partnerId
    );
  }
  ...
  getPrimaryActionLabel() {
    ...
    const usableVariants = brandName
      ? variants
      : variants.filter((variant) => !variant.includes("{brand}"));
    const chosenVariant =
      usableVariants[this.getStableVariantIndex(usableVariants.length)] ||
  ...
  getAudienceLabel() {
    ...
    if (
      ...
      return label(
        "Für Stream, Video & Workflow",
        "For stream, video & workflow",
      );
    }
  ...
  hasAiAssistedImage() {
    return ["social-wish", "campaign-wish", "donation-wish"].includes(
      this.adHost,
    );
  }
  ...
  createCard() {
    ...
    mediaLink.setAttribute(
      "title",
      cardContent.primaryActionTitle || mediaAction,
    );
    ...
    const resolvedImage =
      this.resolveAssetPath(this.image) ||
      window.WishlistUi.resolveLocalPath("assets/images/gridAdCardLoader.gif");
    ...
    image.onerror = () => {
      ...
      image.onerror = null;
      image.src = window.WishlistUi.resolveLocalPath(
        "assets/images/gridAdCardLoader.gif",
      );
    ...
    meta.textContent =
      this.status === "gifted" ? this.statusMeta : this.getAudienceLabel();
    ...
    copyButton.setAttribute(
      "aria-label",
      cardContent.shareActionTitle || this.getShareActionLabel(),
    );
    copyButton.setAttribute(
      "title",
      cardContent.shareActionTitle || cardContent.shareAction || "Teilen",
    );
    ...
    const shareMenuIndex = (AdCard.shareMenuIdCounter =
      (AdCard.shareMenuIdCounter || 0) + 1);
    ...
    copyButton.addEventListener("click", () =>
      this.toggleShareMenu(shareWrapper, copyButton),
    );
    ...
    if (this.status === "gifted") {
      ...
      let giftedLabel =
        this.statusLabel.replace(/^🎁\s*/, "") ||
        cardContentGifted.giftedLabel ||
        "Schon geschenkt";
      if (
        cardContentGifted.giftedLabel &&
        /schon geschenkt/i.test(giftedLabel)
      ) {
    ...
    if (
      this.status === "gifted" &&
      this.content.card &&
      this.content.card.giftedMeta
    ) {
      const germanDefaultMeta = /danke fürs möglichmachen/i.test(
        this.statusMeta || "",
      );
    ...
    if (this.status !== "gifted") {
      const badge = document.createElement(
        this.hasAiAssistedImage() ? "a" : "span",
      );
      badge.className = "product-badge";
      if (this.hasAiAssistedImage()) {
        badge.classList.add("has-ai-content");
        badge.href =
          cardContent.aiContentUrl || "https://cannachris.de/ki-content/";
        badge.target = "_blank";
        badge.rel = "noopener noreferrer";
        badge.setAttribute(
          "aria-label",
          cardContent.aiContentAria ||
            "Wunsch - Hinweis zu KI-Content auf Cannachris öffnen",
        );
        badge.setAttribute(
          "title",
          cardContent.aiContentTitle ||
            cardContent.aiContentAria ||
            "KI-Content",
        );
      } else {
      ...
      badgeText.textContent = this.badgeLabel || cardContent.badge || "Wunsch";
      ...
      if (this.hasAiAssistedImage()) {
        ...
        aiIcon.src = window.WishlistUi.resolveLocalPath(
          "assets/icons/cannachris-icon-okai.svg",
        );
        aiIcon.alt = "";
        aiIcon.width = 16;
        aiIcon.height = 16;
  ...
  createShareMenu(menuId, buttonId, triggerButton, wrapper) {
    ...
    const shareText =
      (cardContent.shareTextPrefix || "Geschenkidee für Chris:") +
      " " +
      this.title;
    ...
    shareTargets.forEach((target) => {
      ...
      link.rel = target.url.startsWith("mailto:")
        ? ""
        : "noopener noreferrer sponsored";
      link.setAttribute("role", "menuitem");
      link.setAttribute(
        "aria-label",
        (cardContent.shareViaAria || "Über {service} teilen").replace(
          "{service}",
          target.label,
        ),
      );
    ...
    copyLink.setAttribute(
      "aria-label",
      cardContent.copyLinkAria || "Direkten Link zur Wunschkarte kopieren",
    );
    copyLink.tabIndex = -1;
    ...
    copyLink.addEventListener("click", () =>
      this.copyProductLink(triggerButton, wrapper),
    );
  ...
  toggleShareMenu(wrapper, button) {
    ...
    document
      .querySelectorAll(".product-share.is-open")
      .forEach((openWrapper) => {
  ...
  async copyProductLink(button, wrapper) {
    ...
    try {
      ...
      setButtonLabel(
        (this.content.card && this.content.card.copySuccess) || "Kopiert",
        "fas fa-check",
      );
      ...
      setButtonLabel(
        (this.content.card && this.content.card.copyError) || "Fehler",
